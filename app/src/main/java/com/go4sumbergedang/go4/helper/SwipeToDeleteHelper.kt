package com.go4sumbergedang.go4.helper
...
abstract class SwipeToDeleteHelper(
    ...
    var swipePosition = -1
    var swipeTreshold = 0.5f
    var buttonBuffer: MutableMap<Int, MutableList<MyButton>>
    lateinit var removeQueue: LinkedList<Int>

    abstract fun instantlateMybutton(viewHolder: RecyclerView.ViewHolder, buffer: MutableList<MyButton>)

    private val gestureLintener = object : GestureDetector.SimpleOnGestureListener(){
    ...
    init {
        ...
        this.gestureDetector = GestureDetector(context,gestureLintener)
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        ...
        if(buttonBuffer.containsKey(swipePosition))
            ...
            buttonList!!.clear()
        buttonBuffer.clear()
        swipeTreshold = 0.5f*buttonList!!.size.toFloat()*buttonWidth.toFloat()
        recoverSwipeItem()
    }
    ...
    override fun onChildDraw(
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE)
        {
            if (dX < 0)
            {
                ...
                {
                    instantlateMybutton(viewHolder, buffer)
    ...
    private fun drawButton(c: Canvas, itemView: View, buffer: MutableList<MyButton>, pos: Int, transitionx: Float) {
